<template>
  <main class="chat-page">
    <header class="chat-header">
      <button type="button" class="back-button" aria-label="返回私信列表" @click="router.back()">‹</button>
      <ProfileAvatar :pubkey="peerPubkey" :local-name="localName" :size="36" />
...
<style scoped>
.chat-page{position:fixed;inset:0;z-index:1000;display:grid;grid-template-rows:auto minmax(0,1fr) auto auto;background:#fff}.chat-header{display:grid;grid-template-columns:42px 36px minmax(0,1fr);align-items:center;gap:9px;min-height:58px;padding:0 12px;border-bottom:1px solid #e9edf2;background:#fff}.back-button{width:40px;height:44px;padding:0;border:0;background:transparent;color:#273344;font-size:31px}.chat-header strong{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:16px}.message-list{min-height:0;overflow-y:auto;padding:14px 12px 18px;overscroll-behavior:contain}.relationship-notice,.empty-chat{margin:14px auto;padding:9px 13px;color:#7a8492;font-size:12px;text-align:center}.message-time{display:block;margin:16px 0 10px;color:#9aa3af;font-size:11px;text-align:center}.message-line{display:flex;align-items:flex-end;gap:6px;margin:3px 0}.message-line.own{justify-content:flex-end}.avatar-slot{display:flex;width:28px;flex:0 0 28px}.message-bubble{max-width:min(76%,430px);padding:9px 12px;border-radius:18px 18px 18px 5px;background:#f0f2f5;color:#202938;line-height:1.45;overflow:hidden}.message-line.own .message-bubble{border-radius:18px 18px 5px 18px;background:#d9efff}.bubble-text{display:block;white-space:pre-wrap;overflow-wrap:anywhere;font-size:15px}.message-bubble :deep(.post-image-preview){margin:-9px -12px}.message-bubble :deep(.carousel-shell){border-radius:16px}.selected-image{position:relative;width:70px;height:70px;margin:6px 14px}.selected-image img{width:100%;height:100%;object-fit:cover;border-radius:10px}.selected-image button{position:absolute;top:-6px;right:-6px;width:22px;height:22px;padding:0;border:0;border-radius:50%;background:#263241;color:#fff}.send-error{padding:4px 14px;color:#dc2626;font-size:11px;text-align:center}.chat-composer{display:grid;grid-template-columns:42px minmax(0,1fr) auto;align-items:center;gap:7px;padding:8px 10px calc(8px + env(safe-area-inset-bottom));border-top:1px solid #e5e9ee;background:#fff}.chat-composer input[type=text]{min-width:0;height:42px;padding:0 14px;border:1px solid #dbe2ea;border-radius:999px;font-size:16px}.image-input{display:none}.image-button,.send-button{height:42px;border:0;background:transparent;color:#1687e8;font-weight:700}.image-button{width:42px;padding:9px}.image-button svg{display:block;width:23px;height:23px;fill:none;stroke:currentColor;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round}.send-button{padding:0 5px}.chat-composer button:disabled{opacity:.4}
</style>
