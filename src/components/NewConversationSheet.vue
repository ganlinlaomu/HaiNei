      ...
      <div v-if="visible" class="sheet-backdrop" role="presentation" @click.self="emit('close')">
        <section class="sheet-panel" role="dialog" aria-modal="true" aria-labelledby="new-conversation-title">
          ...
          <div class="friend-list">
            ...
            <div v-if="!loading && filteredFriends.length === 0" class="empty-friends">
...
async function load() {
  ...
  try {
    ...
    if (keys.pkHex === account) loading.value = false;
  }
}
...
.friend-list{min-height:120px;overflow-y:auto}.friend-row{display:grid;grid-template-columns:44px minmax(0,1fr) 20px;align-items:center;gap:12px;width:100%;min-height:66px;padding:10px 16px;border:0;border-bottom:1px solid #eff1f3;background:#fff;color:#0f1419;text-align:left}.friend-row:active{background:#f7f9f9}.friend-copy{display:flex;min-width:0;flex-direction:column;gap:3px}.friend-copy strong,.friend-copy small{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.friend-copy strong{font-size:15px}.friend-copy small{color:#536471;font-size:12px}.row-arrow{width:18px;height:18px;color:#8b98a5}.empty-friends{padding:36px 20px 48px;color:#536471;font-size:14px;text-align:center}
...
@media (min-width:720px){.sheet-panel{max-width:620px;margin-bottom:24px;border-radius:20px}}
</style>
